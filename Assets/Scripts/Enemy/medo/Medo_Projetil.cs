...
public class Medo_Projetil : MonoBehaviour
{
    ...
    // variavel para empurrar o player pra longe da escuridao
    Player_Movement pm;
    float empurrao = 7.0f;
    ...
    void Awake()
    {
        ...
        if (Panel == null)
        {
            GameObject panelObj = GameObject.FindGameObjectWithTag("escuridao");
    ...
    void Start()
    {
        ...
        Panel.color = new Color(0, 0, 0, 0);//uma garantia que sera preto no inicio do jogo
    ...
    // NECESSITA DE ATUALIZAÇÃO NA ESTRUTURA, O PLAYER FICA PERMANENTEMENTE TRAVADO EM CASO DE COLISAO
    // CRIAR UM IENUMERATOR DENTRO DO PLAYER QUE CANCELA O MOVIMENTO, MAIS SEGURO
    IEnumerator knockback(Vector2 direcao, float travamentoTempo)
    {
        ...
        //pm.enabled = false; NAO TA FUNCIONANDO O TRAVAMENTO
        yield return new WaitForSeconds(travamentoTempo);
        //pm.enabled = true;
        float alphaAtual = Panel.color.a;
        float velocidadeFade = 2f; // Quanto maior, mais rápido
    
        while (alphaAtual > 0.01f)
        {
            alphaAtual -= velocidadeFade * Time.deltaTime;
            Panel.color = new Color(0, 0, 0, alphaAtual);
            yield return null;
        }
